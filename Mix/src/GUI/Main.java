    ...
    private void compileMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_compileMenuItemActionPerformed
        ...
        if (((FileFrame)desktopPane.getSelectedFrame()).getPreviouslySaved()) {
            ...
            if (compiler.compileProgram(desktopPane.getSelectedFrame().getTitle())) {           
                ...
                disassembler.Disassemble(desktopPane.getSelectedFrame().getTitle().replace(".tri", ".tam")); //se descomenta para generar el .tam
                ((FileFrame)desktopPane.getSelectedFrame()).setTree((DefaultMutableTreeNode)treeVisitor.visitProgram(compiler.getAST(), null));//se descomenta para crear el table details
                ((FileFrame)desktopPane.getSelectedFrame()).setTable(tableVisitor.getTable(compiler.getAST()));        //       
                runMenuItem.setEnabled(false); //era true
                buttonRun.setEnabled(false); //era true
            } else {
